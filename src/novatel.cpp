...
void Novatel::ReadSerialPort() {
	...
	while (reading_status_) {
		try {
			...
			std::stringstream debuglen;
			debuglen << "read len=" << len;
			log_debug_(debuglen.str());
		} catch (std::exception &e) {
	        std::stringstream output;
...
//                log_info_("RECEIVER RESET COMPLETE RECEIVED.");
		...
		// 	bytes_remaining_ = message[ii] + 4 + (header_length_-7) - 2;
			...
			// JMS HACK figure out why this is not being called, but parse binary is
			log_debug_("Sending to ParseBinary");
